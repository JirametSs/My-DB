...
<main>
    <nav>
        <a href="{{ route('categories.view', ['category' => $category->code]) }}" class="back-link">
            &lt; Back
        </a>
    </nav><br><br>
    ...
    <form action="{{ route('categories.view-products', ['category' => $category->code]) }}" method="get" class="search-form">
        ...
        <div>
            ...
            <input id="app-inp-min-price" type="number" name="minPrice" value="{{ $search['minPrice'] ?? '' }}" placeholder="Min"/>
        </div><br>

        <div>
            <label for="app-inp-max-price">Max Price ::</label>
            <input id="app-inp-max-price" type="number" name="maxPrice" value="{{ $search['maxPrice'] ?? '' }}" placeholder="Max"/>
        </div><br>
            ...
            @foreach($products as $product)
                <tr>
                    ...
                    <td>{{ $product->category ? $product->category->name : 'No Category' }}</td>
                        <td>
                        <!-- Add Product Form -->
                        <!-- Instead of submitting the form, redirect to add-products-form -->
                            <a href="{{ route('categories.add-products-form', ['category' => $category->code]) }}" class="button">
                                Add
                            </a>
                        </td>
                    </td>
